#!/bin/bash 
#SBATCH --job-name=HUMAN_array
#SBATCH --output=HUMAN_%A_%a.out  # Unique output for each job
#SBATCH --error=HUMAN_%A_%a.err   # Unique error log for each job
...
if [ -d "$D_PATH" ]; then
    srun singularity exec --bind /share/proteomics/Data/lab/service/off_campus/NIST/bat_sparrow_project_2024_2025/:/share/proteomics/Data/lab/service/off_campus/NIST/bat_sparrow_project_2024_2025/ /share/proteomics/Data/lab/service/off_campus/NIST/bat_sparrow_project_2024_2025/dia-nn/diann-2.0.1.sif /diann-2.0.1/diann-linux --f "$D_PATH" --lib \"$LIB_PATH\" --threads 20 --verbose 1 --qvalue 0.01 --fasta \"$FASTA_PATH\"--met-excision --cut K*,R* --missed-cleavages 1 --unimod4 --var-mods 1 --mass-acc 15 --mass-acc-ms1 15 --window 6 --rt-profiling --pg-level 1 --quant-ori-names 
else
